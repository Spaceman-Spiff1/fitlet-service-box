...
|-- bundle/
|   `-- README.md
|-- dist/ (generated by packaging, not tracked)
|-- docs/
...
- a package-build job that creates and validates installer artifacts
...
On tag pushes like `v1.0.0`, `.github/workflows/release.yml` builds installer assets and publishes them to GitHub Releases.
That gives you a downloadable `.run` installer, a `.tar.gz` source package, and checksums without storing bundle payloads in git history.

## Release Installer

This repo can now produce GitHub-hosted installer assets:
...
The release installer is built from git-tracked content, so it does not include large local `bundle/` payloads by default.
For offline or USB-heavy installs, put these sidecar items next to the `.run` file on the USB drive:
- `.env`
- `bundle/`

When the `.run` installer starts, it copies any sidecar `.env` and `bundle/` directory into its extracted workspace before `install.sh` runs.
That keeps the GitHub-built package practical without pushing packages or image tarballs into the repo.
...
If you prefer a single downloaded artifact, you can also use the GitHub release installer:

```bash
sudo ./fitlet-service-box-installer-<version>.run
```

If `.env` and `bundle/` are present beside that `.run` file, the installer uses them automatically.
...
If you are using a GitHub release installer instead of the full repo tree, copy `bundle/` beside the `.run` file on the USB drive instead.
